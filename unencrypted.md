...
lvcreate --name swap --size 4GB myvolgroup
lvcreate --name root --extents '100%FREE' myvolgroup
```
...
mkfs.ext4 -L root /dev/myvolgroup/root
mkswap -L swap /dev/myvolgroup/swap
```
...
Exit root account and login as guest.
...
Log into Gnome as guest. Connect to wifi.
